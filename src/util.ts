...
export const addIconToTab = (plugin: IconFolderPlugin, file: TFile) => {
  // Checks if tab header element exists with the `aria-label` attribute.
  const node = document.querySelector(`[aria-label="${file.basename}"]`);
...
export const removeIconFromTab = (plugin: IconFolderPlugin, file: TFile) => {
  // Checks if tab header element exists with the `aria-label` attribute.
  const node = document.querySelector(`[aria-label="${file.basename}"]`);
  if (!node || !node.hasAttribute('draggable') || node.children.length === 0) {
    return;
